...
async function playtop(client, message, args, type) {
    ...
    //otherwise
     ...
     //save old tracks on an var
      ...
      for (const track of oldQueue)
        player.queue.add(track);
      let playembed = new Discord.MessageEmbed()
      .setAuthor(`Added to queue`, message.author.displayAvatarURL({dynamic: true}), "https://echobot.gq")
      .setURL(res.tracks[0].uri)
      .setTitle("**" + res.tracks[0].title + "**").setColor(ee.color)
      .setThumbnail(`https://img.youtube.com/vi/${res.tracks[0].identifier}/mqdefault.jpg`)
      .addField("Channel", res.tracks[0].author, true)
  ...
  function playlist_() {
    ...
    for(const track of res.tracks)
      if(track.duration > 86400){
        return message.channel.send(`**:x: Cannot play a song that's longer than 1 day --> playlist skipped!**`)
      }
    let player;
    player = client.manager.create({
      guild: message.guild.id,
